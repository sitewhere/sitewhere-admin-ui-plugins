<template>
  ...
  >
      ...
      <dialog-header>
        <v-layout class="pl-4 pr-4 pt-0 pb-0" row wrap>
          <v-flex xs12>
            <form-text
              v-if="visible"
              label="Outbound connector id"
              title="Unique outbound connector identifier."
              v-model="id"
              icon="info"
              :dense="true"
              :autofocus="true"
    ...
    <template slot="tabs">
      <slot name="outbound-connector-tabs" />
    </template>
    <template slot="tab-items">
      <slot name="outbound-connector-tab-items" />
    </template>
  </base-dialog>
</template>
...
export default class OutboundConnectorDialog extends DialogComponent<
